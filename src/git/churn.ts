...
export function fileChurn(
    ...
    // Audit defense-in-depth — same allowlist gate as repoChurn (no-op unless
    // configured; no MCP caller today, but keep parity so a future one is safe).
    if (scanPathError(repoRoot)) return { commits: 0, additions: 0, deletions: 0 };
    const result = spawnSync(
        'git',
        ['log', `--since=${sinceDays} days ago`, '--numstat', '--format=', '--', filePath],
        { cwd: repoRoot, encoding: 'utf-8', maxBuffer: 16 * 1024 * 1024, timeout: 30000 },
    );
...
export function repoChurn(repoRoot: string, sinceDays = 30): Map<string, ChurnStats> {
    ...
    // Audit ATL-005 — refuse to spawn git in a non-allowlisted dir (no-op unless
    // ATLAS_INDEX_ROOTS / config.index.roots is set). Empty churn == non-git dir,
    // so callers degrade gracefully.
    if (scanPathError(repoRoot)) return out;
    const result = spawnSync(
        'git',
        ['log', `--since=${sinceDays} days ago`, '--numstat', '--format=__COMMIT__'],
        { cwd: repoRoot, encoding: 'utf-8', maxBuffer: 64 * 1024 * 1024, timeout: 30000 },
    );
