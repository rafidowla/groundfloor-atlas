...
export const walk: WalkerFn = (rootNode, sourceUtf8, file) => {
    ...
    walkSubtree(rootNode, (node) => {
        ...
        if (!owner) return;
        calls.push(...extractCallsInBody(body, owner.id, GO_CALL_NODE_TYPES, extractGoCallee, GO_SCOPE_TYPES));
    });
...
const GO_SCOPE_TYPES: ReadonlySet<string> = new Set([
...
function extractGoCallee(node: Node): { name: string; isMethod: boolean; receiver: string | null } | null {
