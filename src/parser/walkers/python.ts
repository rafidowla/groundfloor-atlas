...
function unwrapDecorated(node: Node): Node {
    if (node.type === 'decorated_definition') {
        const inner = node.childForFieldName('definition')
            ?? node.namedChildren.find((n: Node) =>
        ...
        if (child.type === 'import_statement') {
            ...
            for (let j = 0; j < child.namedChildCount; j++) {
                ...
                if (sub.type === 'dotted_name' || sub.type === 'identifier') {
                    ...
                    const inner = sub.namedChildren.find((n: Node) =>
...
const PYTHON_SCOPE_TYPES: ReadonlySet<string> = new Set([
    'function_definition'
]);

const PY_CALL_NODE_TYPES: ReadonlySet<string> = new Set(['call']);

function extractPyCallee(node: Node): { name: string; isMethod: boolean; receiver: string | null } | null {
...
export const walk: WalkerFn = (rootNode, sourceUtf8, file) => {
    ...
    walkSubtree(rootNode, (node) => {
        ...
        if (!owner) return;
        calls.push(...extractCallsInBody(body, owner.id, PY_CALL_NODE_TYPES, extractPyCallee, PYTHON_SCOPE_TYPES));
    });
