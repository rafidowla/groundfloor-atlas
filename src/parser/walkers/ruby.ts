...
function isModuleConstant(name: string): boolean {
    ...
    // module-level constants (matching the standard filter convention); class
...
export const walk: WalkerFn = (rootNode, sourceUtf8, file) => {
    ...
    walkSubtree(rootNode, (node) => {
        ...
        if (!owner) return;
        calls.push(...extractCallsInBody(body, owner.id, RUBY_CALL_NODE_TYPES, extractRubyCallee, RUBY_SCOPE_TYPES));
    });
...
const RUBY_SCOPE_TYPES: ReadonlySet<string> = new Set([
    'method',
    'singleton_method'
]);
...
function extractRubyCallee(node: Node): { name: string; isMethod: boolean; receiver: string | null } | null {
