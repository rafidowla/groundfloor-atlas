...
export const walk: WalkerFn = (rootNode, sourceUtf8, file) => {
    ...
    walkSubtree(rootNode, (node) => {
        ...
        if (!owner) return;
        calls.push(...extractCallsInBody(body, owner.id, RUST_CALL_NODE_TYPES, extractRustCallee, RUST_SCOPE_TYPES));
    });
...
const RUST_SCOPE_TYPES: ReadonlySet<string> = new Set([
    'function_item'
]);
...
function extractRustCallee(node: Node): { name: string; isMethod: boolean; receiver: string | null } | null {
