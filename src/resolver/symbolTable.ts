...
export interface SymbolTable {
    ...
    /**
     * RD-P2 — workspace-wide bare-name index: localName → all symbols with
     * that name across every file. Built once so the resolver's bare-name
     * fallback is an O(1) map lookup instead of an O(files) scan per call.
     */
    readonly byName: Map<string, ParsedSymbol[]>;
}
...
export function buildSymbolTable(files: readonly ParsedFile[]): SymbolTable {
    ...
    for (const file of files) {
        ...
        for (const symbol of file.symbols) {
            ...
            // RD-P2 — workspace-wide bare-name index (one pass).
            const nameList = byName.get(symbol.name);
            if (nameList) nameList.push(symbol);
            else byName.set(symbol.name, [symbol]);
        }
    }

    return { byQualifiedName, byFile, byId, byName, all };
}
